...
class ComplexConvRealWeight2d(tf.keras.layers.Layer):
    def __init__(self, in_channels, out_channels, kernel_size=3,
                 stride=1, dilation=1, bias=False, 
                 zero_mean=True, bound_norm=True, pad=True):
...
class ComplexConvRealWeight2dTest(unittest.TestCase):
    def test_grad(self):
        nBatch = 5
        M = 256
        N = 256
        ...
        model = ComplexConvRealWeight2d(nf_in, nf_out, kernel_size=3)
        x = tf.complex(tf.random.normal(shape), tf.random.normal(shape))
        Kx = model(x)
