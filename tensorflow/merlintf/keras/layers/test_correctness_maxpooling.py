    ...
    def call(self, x):
        if self.optox:
            if merlintf.iscomplextf(x):
                ...
                x_pool, x_pool_index = optotf.maxpooling.maxpooling2d(x, pooling=self.pool_size, stride=self.strides,
                                                           alpha=self.alpha, beta=self.beta, mode=self.padding)
                if self.argmax_index:
                    return x_pool, x_pool_index
                else:
                    return x_pool
            else:
                x_pool, x_pool_index  = tf.nn.max_pool_with_argmax(x, ksize=self.pool_size, strides=self.strides, padding=self.padding)
                if self.argmax_index:
                    return x_pool, x_pool_index
                else:
                    return x_pool
...
class MagnitudeMaxPool3D(MagnitudeMaxPool):
    ...
    def call(self, x):
        if self.optox:
            if merlintf.iscomplextf(x):
                x_pool, x_pool_index = optotf.maxpooling.maxpooling3d(x, pooling=self.pool_size, stride=self.strides,
                                                           alpha=self.alpha, beta=self.beta, mode=self.padding)
                ...
                if self.argmax_index:
                    x_pool, x_pool_index  = tf.nn.max_pool3d(x, ksize=self.pool_size, strides=self.strides, padding=self.padding)
                    return x_pool, x_pool_index
                else:
...
class MagnitudeMaxPool3Dt(MagnitudeMaxPool):
    ...
    def call(self, x):  # only Optox supported
        ...
        if merlintf.iscomplextf(x):
            x_pool, x_pool_index= optotf.maxpooling.maxpooling3d(x, pooling=self.pool_size, stride=self.strides, alpha=self.alpha,
...
def index_transfer(index_input,index_1,include_batch_in_index=False):
                    ...
                    for w in range(width_o):
                        for d in range(depth_o):
                            for c in range(channel_o):
                                ...
                                w1 = temp1% width
                                h1 = (temp1-w1)// width
                                out_index[b, h, w, d,c] = ((((b * height + h1) * width + w1))*depth+d) * channel_o + c
                    ...
                    for d in range(depth_o):
                        for w in range(width_o):
                            for c in range(channel_o):
                                ...
                                w1 = temp1% width
                                h1 = (temp1-w1)// width
                                out_index[b, h, w, d,c] = (((( height + h1) * width + w1))*depth+d) * channel_o + c
...
def test_2d_corr(shape, pool_size=(2, 2), strides=(2, 2)):
    print('_______')
    print('test_2d_corr...')
    ...
    x_abs = tf.math.abs(x)
    x_abs,out_ind_nn =tf.nn.max_pool_with_argmax(x_abs,pool_size,strides,padding='SAME',include_batch_in_index=True)
    print(out_ind_nn)
...
def test_3d_corr(shape, pool_size=(2, 2,2), strides=(2, 2,2)):
    print('_______')
    print('test_3d_corr...')
    x = tf.complex(tf.random.normal(shape), tf.random.normal(shape))
...
test_2d_corr([1, 8, 12, 3],pool_size=(3,2)) # input shape: [batch, height, width, channel]
test_3d_corr([2, 8, 6, 8, 2],pool_size=(3,2,2))  # input shape: [batch, height, width, depth, channel]
