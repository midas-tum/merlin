...
def validate_input_dimension(dim, param):
    ...
    if isinstance(param, tuple) or isinstance(param, list):
        if not len(param) == n_dim:
            raise RuntimeError(
                "Parameter dimensions {} do not match requested dimensions {}!".format(len(param), n_dim))
        else:
            return param
    else:
...
model = Conv3Dt(nf_out, nf_inter, kernel_size=ksz, shapes=shape, axis_conv_t=3)
...
model2 = Conv3DtTranspose(nf_out, nf_inter, kernel_size=ksz, shapes=shape, axis_conv_t=3, strides=(2, 2, 2, 2))  # strides =2
x_real = tf.cast(tf.random.normal(shape), dtype=tf.float32)
x_imag = tf.cast(tf.random.normal(shape), dtype=tf.float32)
x = tf.complex(x_real, x_imag)
Kx = model2(x).numpy()
