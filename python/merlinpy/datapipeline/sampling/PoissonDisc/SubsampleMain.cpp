...
int main(int argc,char *argv[])
{
    ...
    if(lPerform)
    {
        ...
        {
            int ***samplingMask = startPD(nX, nY, accels, fully_sampled, pF_value, pF_x, lPhases, vd_type, smpl_type, ellip_mask, p, n, body_region, iso_fac, lVerbose);
        } else if(subsampleType == 2) // Gaussian
        {
            int ***samplingMask = startGaussian(nX, nY, accels, fully_sampled, pF_value, pF_x, long(lPhases), lVerbose);
        }
    }
...
void run_subsampling(double* kSpacePolar_LinInd, double* kSpacePolar_ParInd, double* phaseInd, long nX, long nY, double accels, short int subsampleType, float fully_sampled, float pF_value, bool pF_x, int lPhases, short int vd_type, short int smpl_type, bool ellip_mask, float p, float n, short int body_region, float iso_fac, bool lVerbose)
{
    if(lVerbose)
        cout << "Starting subsampling" << endl;
    int ***samplingMask;
    if(subsampleType == 1) // Poisson-disc
    {
        samplingMask = startPD(nX, nY, accels, fully_sampled, pF_value, pF_x, lPhases, vd_type, smpl_type, ellip_mask, p, n, body_region, iso_fac, lVerbose);
    } else if(subsampleType == 2) // Gaussian
    {
        samplingMask = startGaussian(nX, nY, accels, fully_sampled, pF_value, pF_x, long(lPhases), lVerbose);
    }
    if(lVerbose)
        cout << "...subsampling finished" << endl;
    ...
    for (long i = 0; i < nX; i++)
    {
        for (long j = 0; j < nY; j++)
        {
            ...
            {
                if(samplingMask[i][j][k] > 0)
                {
                    kSpacePolar_LinInd[iCnt] = double(i);
                    kSpacePolar_ParInd[iCnt] = double(j);
                    phaseInd[iCnt] = double(k);
                    iCnt++;
                }
            }
        }
...
int ***startPD(long lLines, long lPartitions, double dAccel, float fully_sampled, float pF_val, bool pFx, int lPhases, short int v_type, short int s_type, bool ellipticalMask, float p, float n, short int body_part, float iso_fac, bool lVerbose)
{
    //////////////////////////////////////////
...
int ***startGaussian(long lLines, long lPartitions, double dAccel, float fully_sampled, float pF_val, bool pFx, long lPhases, bool lVerbose)
{
